...
#--------Install Azure AD module and Login Not used currently----#

<#
if (Get-Module -ListAvailable -Name AzureAD) {
    Write-Host "Login to Azure..."
    Connect-AzureAD | Out-Host
} else {
...
function CreateUser {   
    ...
    $oldUsers = Get-ADUser -Filter * -SearchBase "DC=DOMAIN,DC=COM" | Select sAMAccountName
    ...
    if($cuser[$loop] -ne ""){
        ...
        $testOU = "OU=Users,OU=" + $copy.City + ",OU=OU,dc=domain,dc=com"
        New-ADuser -Name $fullname -EmailAddress $email -SamAccountName $user -DisplayName $fullname -GivenName $fname[$loop] -Surname $lname[$loop] -UserPrincipalName $email -Instance $copy -Accountpassword (ConvertTo-SecureString -AsPlainText "Password123!" -Force) -Enabled $true -Path $testOU
    }else{
